...
func (gss *GameServerSet) GameServer() *GameServer {
	gs := &GameServer{
		ObjectMeta: *gss.Spec.Template.ObjectMeta.DeepCopy(),
		Spec:       *gss.Spec.Template.Spec.DeepCopy(),
	}

	// Switch to GenerateName, so that we always get a Unique name for the GameServer, and there
	// can be no collisions
	gs.ObjectMeta.GenerateName = gss.ObjectMeta.Name + "-"
	gs.ObjectMeta.Name = ""
	gs.ObjectMeta.Namespace = gss.ObjectMeta.Namespace
	gs.ObjectMeta.ResourceVersion = ""
	gs.ObjectMeta.UID = ""

	ref := metav1.NewControllerRef(gss, SchemeGroupVersion.WithKind("GameServerSet"))
	...
	gs.ObjectMeta.Labels[GameServerSetGameServerLabel] = gss.ObjectMeta.Name
